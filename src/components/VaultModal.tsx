...
import { X, Key, ShieldCheck, Palette, Share2, Check, Lock, RefreshCw } from "lucide-react";
...
export default function VaultModal() {
  ...
  const handleSaveKey = async (provider: "openai" | "anthropic" | "replicate" | "fal") => {
    ...
    try {
      ...
      alert(`API Key for ${provider.toUpperCase()} saved securely in Vault!`);
    } catch (err: any) {
      ...
      <div className="bg-[#F7F7F5] w-full max-w-2xl rounded-3xl border-2 border-gray-300 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 bg-white border-b border-gray-200 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-[#3B28CC] flex items-center justify-center text-white">
              <ShieldCheck className="w-5 h-5 text-[#D4FF33]" />
            </div>
            <div>
        ...
        <div className="p-6 overflow-y-auto space-y-6">
            ...
            <div className="space-y-4">
              <div className="p-3.5 rounded-2xl bg-[#3B28CC]/5 border border-[#3B28CC]/20 flex items-center gap-3">
                ...
                <p className="text-xs text-gray-700 font-medium">
                  Your keys are encrypted with <strong>AES-256-GCM</strong> before hitting the database. They are only decrypted at runtime during generation.
                </p>
              </div>

              {[
                { provider: "openai", name: "OpenAI API Key", desc: "For GPT-4o streaming text generation" },
                ...
                { provider: "fal", name: "fal.ai API Key", desc: "For high-speed image generation" },
                ...
                return (
                  <div key={provider} className="p-4 rounded-2xl bg-white border border-gray-200 space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {savingKey === provider ? "Saving..." : "Save Key"}
                      </button>
                    </div>
                  </div>
                );
          ...
          {activeTab === "socials" && (
            <div className="space-y-4">
              ...
              {[
                { key: "x", name: "X (Twitter) API OAuth", icon: "🐦" },
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          {isConnected ? "Connected with OAuth token" : "Not connected"}
                        </p>
                      </div>
                    </div>

                    <button
                      onClick={() => setSocialConnected(key as any, !isConnected)}
